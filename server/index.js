...
io.on('connection', (socket) => {
  ...
  socket.on('del chat', (body) => {
    const { chatId } = body;
    chats.remove({_id: chatId }, {}, (err, numRemoved) => {
      socket.broadcast.emit('del chat', chatId);
      socket.emit('del chat', chatId);
    });
  });
  ...
  socket.on('chat message', (body) => {
    ...
    chats.update({ _id: chatId }, { $push: { messages: message } }, {}, () => {
      ...
      setTimeout(() => {
        const botMessage = { author: 'Bot', text: `Привет, ${body.author}! Бот на связи!`, timeStamp: new Date() };

        chats.update({ _id: chatId }, { $push: { messages: botMessage } }, {}, () => {
          socket.broadcast.emit('chat message', { ...botMessage, chatId });
          socket.emit('chat message', { ...botMessage, chatId });
        });
      }, 5000);
    });
  });
